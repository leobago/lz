...
{
    ulong byteStats[256], i = 0, code = 0, step = 1, max = 0, limit = size;
    if (size > MAX_STATS)
    { // Analyse maximum MAX_STATS bytes
        step = size/MAX_STATS;
        limit = MAX_STATS;
    }
    for (i = 0; i < 256; i++) byteStats[i] = 0;
    while (i < size)
    ...
    { // Convert counters into percentage and get percentage of the most frequent
        byteStats[i] = (byteStats[i]*100)/limit;
...
int lzCompressFlopnt(uchar *dstBuf, ulong *outSize, uchar **tmpBuf, ulong offset, ushort prec, short level, short lossy)
{
    ...
    //printf(" -Byte compression layout: ");
    for (i = 0; i < prec; i++)
    {
        ...
        //printf("%d, ", code[i]);
        memcpy(dstBuf+finalSize, code+i, sizeof(short));
        finalSize = finalSize + sizeof(short);
        if (code[i] > 0)
        { // Bytes that need to be compressed
            gettimeofday(&start, NULL);
            *outSize = offset;
            lzCompress(xtrBuf, outSize, tmpBuf[i], offset, level);
    ...
    //printf("Entropy time: %f compressing time : %f Copy and discard time %f \n", t0, t1, t2);
    //printf("\n");
    *outSize = finalSize;
    free(xtrBuf);
    return EXIT_SUCCESS;
...
int lzUncompressFlopnt(uchar **tmpBuf, ulong offset, uchar *srcBuf, ulong inSize, ushort size)
{
    ...
    {
        outSize = offset;
        memcpy(code+i, srcBuf+finalSize, sizeof(short));
        finalSize = finalSize + sizeof(short);
        //printf("%d, ", code[i]);
...
int lzCompressDouble(uchar *dstBuf, ulong *outSize, double *daBuf, ulong daSize, short level, short lossy)
{
    ...
    *outSize = offset*size;
    ...
    //printf("Reformatting time: %f, compression time : %f \n", t0, t1);
    for (i = 0; i < size; i++) free(tmpBuf[i]);
    return EXIT_SUCCESS;
}
